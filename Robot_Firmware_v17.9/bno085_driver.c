...
uint32_t valid_channels[MAXIMUM_CHANNELS];
...
uint32_t spec_MaxCargoPlusHeaderWrite = 0;
uint32_t spec_MaxCargoPlusHeaderRead = 0;
uint32_t spec_MaxTransferWrite = 0;
uint32_t spec_MaxTransferRead = 0;
uint32_t spec_SHTPVersion = 0;
...
a uint32_t but is in Least Significant Byte to Most Significant Byte (left to right), as well as the length
(between 0 and 4 inclusively), and returns a uint32_t that represents the 4 ordered correctly to be a uint32_t.
...
uint32_t byte_array_to_uint(uint8_t* byte_array, uint8_t length)
{
	if(length == 1){
		...
		return (uint32_t)((uint32_t)byte_array[0] | ((uint32_t)byte_array[1] << 8));
	}
	else if(length == 3){
		return (uint32_t)((uint32_t)byte_array[0] | ((uint32_t)byte_array[1] << 8) | ((uint32_t)byte_array[2] << 16));
	}
	else if(length == 4){
		return (uint32_t)((uint32_t)byte_array[0] | ((uint32_t)byte_array[1] << 8) | ((uint32_t)byte_array[2] << 16) | ((uint32_t)byte_array[3] << 24));
	}
	else{
		return 0; //Error, uint32_t can only store 4 bytes!
	}
}
	...
	if(valid_channels[my_Header.channel] != 1){
		...
		if(my_Header.channel == 0){ // Entering this means the channel is the command channel
			...
			if(packetPointer[4]==0){ // Entering this means the packet is a SHTP Advertisement (See above)
				...
				but thats a lot of work. 1 byte can only hold a uint32_t of maximum size 256, and because L is
			...
			switch(packetPointer[4]){
				case 252: //0xFC
					if(VERBOSE == 1){
						...
						//printf("Feature Report ID = %u\r\n", (uint32_t)packetPointer[5]);
						//printf("Feature flags = %u\r\n", (uint32_t)packetPointer[6]);
				...
				case 241: //0xF1
					if(VERBOSE == 1){
						...
						//printf("Response Sequence Number = %u\r\n", (uint32_t)packetPointer[8]);
						//printf("R0 Value = %u\r\n", (uint32_t)packetPointer[9]);
						//printf("R1 Value = %u\r\n", (uint32_t)packetPointer[10]);
						//printf("R2 Value = %u\r\n", (uint32_t)packetPointer[11]);
						//printf("R3 Value = %u\r\n", (uint32_t)packetPointer[12]);
						//printf("R4 Value = %u\r\n", (uint32_t)packetPointer[13]);
						//printf("R5 Value = %u\r\n", (uint32_t)packetPointer[14]);
						//printf("R6 Value = %u\r\n", (uint32_t)packetPointer[15]);
						//printf("R7 Value = %u\r\n", (uint32_t)packetPointer[16]);
						//printf("R8 Value = %u\r\n", (uint32_t)packetPointer[17]);
						//printf("R9 Value = %u\r\n", (uint32_t)packetPointer[18]);
						//printf("R10 Value = %u\r\n", (uint32_t)packetPointer[19]);
					}
				break;
				default:
					if(VERBOSE == 1){
			...
			switch(packetPointer[9]){
				memset(&location_of_imu_vector_buffer[0], 0, sizeof(uint32_t)*4);
